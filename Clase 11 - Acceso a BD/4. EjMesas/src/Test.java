...
public class Test {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            logger.error(" Error al generar la Query  : " + e);
            logger.info("ERROOOOOOOOR");
        }

        String consulta = "UPDATE TEST SET name='Giovanni' WHERE name='Ag_Vanetta'";
        stmt.execute(consulta);

        try {
            String borrar = "DELETE FROM TEST WHERE ID=3";
            stmt.execute(borrar);
            logger.info(agustin3.getNombre());
            logger.info("borrar");
        } catch (Exception e){
            logger.error("Error en borrar : "+e);
        }
